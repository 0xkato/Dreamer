...
function ThemeToggle() {
  const { settings, toggleTheme } = useProjectStore();
...
export function AppToolbar({ onSave }: AppToolbarProps) {
  ...
  return (
    <div className="h-12 bg-white dark:bg-slate-800 border-b border-slate-200/80 dark:border-slate-700 flex items-center px-4 gap-1 shadow-sm">
      ...
      <div className="flex items-center gap-2 mr-4">
        ...
        <div className="flex items-center gap-1">
          <span className="font-semibold text-slate-800 dark:text-slate-200 text-sm">Dreamer</span>
          {currentProject && (
            <>
      ...
      {activeFile && (
        <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-700 rounded-lg mr-2">
          ...
          <span className="text-xs font-medium text-slate-600 dark:text-slate-300 truncate max-w-40">
            {activeFile.name}
          </span>
          {activeFile.isDirty && (
