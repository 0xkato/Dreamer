...
function ToolButton({ tool, label, icon, shortcut, disabled }: ToolButtonProps) {
  ...
  return (
    <button
      ...
      title={disabled ? 'Create or open a whiteboard first' : (shortcut ? `${label} (${shortcut})` : label)}
    >
      <div className={`w-8 h-8 flex items-center justify-center rounded-lg ${
        isActive && !disabled ? 'bg-indigo-100' : 'bg-transparent'
      }`}>
        {icon}
      </div>
...
export function Sidebar() {
  const { currentDiagramId } = useElementsStore();
  const isDisabled = !currentDiagramId;
